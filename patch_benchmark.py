...
new_benchmark = """def benchmark(model_path=None):
    ...
    for ip in imgs:
        ...
        # Gerçekçi uçuş koşulları simülasyonu (daha dengeli)
        img = Image.open(ip).convert("RGB")
        if random.random() < 0.2:  # %20 ihtimalle kamera sarsıntısı
            img = img.filter(ImageFilter.GaussianBlur(radius=random.uniform(0.5, 1.2)))
        if random.random() < 0.2:  # %20 ihtimalle parlaması
            img = ImageEnhance.Brightness(img).enhance(random.uniform(1.2, 1.5))
        if random.random() < 0.1:  # %10 renk solması
            img = ImageEnhance.Color(img).enhance(random.uniform(0.5, 0.8))
    ...
    # Veri özeti (support'tan çekelim)
...
content = re.sub(r'def benchmark\(.*?_open_file\(str\(report_path\)\)', new_benchmark, content, flags=re.DOTALL)
