...
try:
    from ml_trainer_v2 import collect_training_data, predict, dataset_summary
    HAS_ML = True
except (ImportError, OSError):
    HAS_ML = False
...
def test_vision_scenario(pencere, scenario: FaultScenario, use_ai):
    ...
    try:
        ...
        if HAS_ML:
            # Veriyi eğitim setine ekle (her test = 1 yeni örnek)
            ...
            if "error" not in pred:
                result.ml_prediction = f"{pred['class']}  %{pred['confidence']*100:.0f}"
                ml_info = f"[ML] {result.ml_prediction}  ({pred['ms']}ms)"
                print(f"       ├ {ml_info}")
                result.ai_response += f"\n{ml_info}"
            else:
                result.ml_prediction = f"Veri toplaniyor ({summary['total']}/30)"
                ml_info = f"[ML] {result.ml_prediction} ornek"
                print(f"       ├ {ml_info}")
                result.ai_response += f"\n{ml_info}" 
...
def pytest_sessionfinish(session, exitstatus):
    # Rapor her durumda üretilir ve açılır (sonuç olmasa bile)
    html_path = os.path.abspath(os.path.join(REPORT_DIR, "report.html"))
    ...
    try:
        ...
        try:
            ...
            print(f"  Raporu manuel açın: {os.path.abspath(html_path)}")
